...
void OrderStorage<Order>::FindMatches() {
  for (auto sale_order_it = MapOrders(TradingSide::kSale).begin();
       sale_order_it != MapOrders(TradingSide::kSale).end();
       MoveMapOrderIter(sale_order_it)) {
    for (auto buy_order_it = MapOrders(TradingSide::kBuy).rbegin();
         buy_order_it != MapOrders(TradingSide::kBuy).rend();
      ...
      if (OrderType::MatchingType::DoMatch(buy_order_it->second,
        ...
        TradingPlatformScope::Get()->CloseDeal<Deal<OrderType>>(
